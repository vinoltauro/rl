...
\begin{tikzpicture}[
  blk/.style={draw, rectangle, rounded corners=4pt,
              minimum width=2.0cm, minimum height=0.9cm,
              font=\normalsize, fill=tcdBlue!10, align=center, inner sep=5pt},
  inp/.style={blk, fill=tcdBlue!22},
  outblk/.style={blk, fill=orange!22},
  actor/.style={blk, minimum width=1.9cm, fill=green!20},
  critic/.style={blk, minimum width=1.9cm, fill=red!14},
  lann/.style={font=\small, align=center},
  >=Stealth, line width=1.2pt
]
%% ── (a) DQN ─────────────────────────────────────────────────
\node[inp]  (d0) at (0.0, 0) {Input\\$n_\text{obs}$};
\node[blk]  (d1) at (2.8, 0) {FC 128\\ReLU};
\node[blk]  (d2) at (5.6, 0) {FC 128\\ReLU};
\node[outblk] (d3) at (8.4, 0) {FC $|A|$};
\draw[->] (d0)--(d1); \draw[->] (d1)--(d2); \draw[->] (d2)--(d3);
\node[lann, above=0.25cm of d0] {State $s$};
\node[lann, above=0.25cm of d3] {$Q(s,a)$};
\node[lann, below=0.35cm of d1, font=\small\bfseries] {\textbf{(a)}\enspace DQN};

%% separator
\draw[densely dotted, gray!60, line width=0.8pt] (-0.5, -1.4) -- (10.0, -1.4);

%% ── (b) Actor-Critic ─────────────────────────────────────────
\node[inp]    (a0) at (0.0, -3.5) {Input\\$n_\text{obs}$};
\node[blk]    (a1) at (2.8, -3.5) {FC 64\\Tanh};
\node[blk]    (a2) at (5.6, -3.5) {FC 64\\Tanh};
\node[actor]  (aa) at (8.8, -2.6) {FC $|A|$\\Softmax};
\node[critic] (ac) at (8.8, -4.4) {FC 1};
\draw[->] (a0)--(a1); \draw[->] (a1)--(a2);
%% fork
\draw[->] (a2.east) -- ++(0.65,0) |- (aa.west);
\draw[->] (a2.east) -- ++(0.65,0) |- (ac.west);
\node[lann, above=0.25cm of a0] {State $s$};
\node[lann, above=0.18cm of aa] {$\pi(a|s)$ \small(Actor)};
\node[lann, below=0.18cm of ac] {$V(s)$ \small(Critic)};
\node[lann, below=0.35cm of a1, font=\small\bfseries]
  {\textbf{(b)}\enspace A2C / PPO Actor-Critic};
\end{tikzpicture}}
\caption{Network architectures. (a)~DQN: two hidden layers of 128 units with ReLU activations mapping the state to per-action Q-values (2 units for CartPole, 3 for MountainCar). (b)~A2C/PPO actor-critic: two hidden layers of 64 units with Tanh activations, branching into a softmax policy head (actor) and a scalar value head (critic). All weights use orthogonal initialisation.}
\label{fig:arch}
\end{figure}
...
\resizebox{\linewidth}{!}{%
\begin{tikzpicture}[
  bx/.style={draw, rectangle, rounded corners=4pt,
             minimum width=3.0cm, minimum height=0.85cm,
             font=\normalsize, fill=tcdBlue!10, align=center, inner sep=5pt},
  buf/.style={bx, fill=orange!20},
  rdbx/.style={bx, fill=red!14},
  grn/.style={bx, fill=green!18},
  hdr/.style={font=\normalsize\bfseries, align=center},
  arr/.style={->, >=Stealth, line width=1.2pt},
  darr/.style={->, >=Stealth, line width=1.2pt, dashed},
  lbl/.style={font=\small, fill=white, inner sep=2pt}
]
%% ── Column 1: DQN (x = 0) ────────────────────────────────────────
\node[hdr]   at (0,  1.0) {DQN};
\node[bx]    (de) at (0,  0.0)  {Environment};
\node[buf]   (db) at (0, -1.6)  {Replay Buffer\\50\,K transitions};
\node[bx]    (dn) at (0, -3.2)  {Online Net $\theta$};
\node[bx]    (dt) at (0, -4.8)  {Target Net $\theta^{-}$};
\draw[arr]  (de)--(db) node[lbl,midway,right]{store $(s,a,r,s')$};
\draw[arr]  (db)--(dn) node[lbl,midway,right]{sample batch};
\draw[darr] (dn)--(dt) node[lbl,midway,right]{copy every 500 steps};

%% ── Column 2: A2C (x = 5.5) ─────────────────────────────────────
\node[hdr]   at (5.5,  1.0) {A2C  (single env)};
\node[bx]    (ae) at (5.5,  0.0)  {Environment};
\node[bx]    (ar) at (5.5, -1.6)  {Episode rollout};
\node[bx]    (au) at (5.5, -3.2)  {GAE advantages};
\node[rdbx]  (ag) at (5.5, -4.8)  {1 gradient update};
\draw[arr] (ae)--(ar); \draw[arr] (ar)--(au); \draw[arr] (au)--(ag);
%% return arrow: right side, well clear of nodes
\draw[darr] (ag.east) -- ++(1.2,0) -- ++(0,4.8) -- (ae.east)
            node[lbl, pos=0.5, xshift=4pt]{discard data};

%% ── Column 3: PPO (x = 11.0) ─────────────────────────────────────
\node[hdr]   at (11.0,  1.0) {PPO};
\node[bx]    (pe) at (11.0,  0.0)  {Environment};
\node[bx]    (pb) at (11.0, -1.6)  {1,024-step buffer};
\node[bx]    (pa) at (11.0, -3.2)  {GAE advantages};
\node[grn]   (pg) at (11.0, -4.8)  {10 epochs\,$\times$\,minibatches};
\draw[arr] (pe)--(pb); \draw[arr] (pb)--(pa); \draw[arr] (pa)--(pg);
%% return arrow: left side, well clear of nodes
\draw[darr] (pg.west) -- ++(-1.2,0) -- ++(0,3.2) -- (pb.west)
            node[lbl, pos=0.5, xshift=-4pt]{reuse data};
\end{tikzpicture}}
\caption{Data collection and update strategies for the three deep RL algorithms. \textbf{DQN} (left) stores every transition in a 50K replay buffer and samples random minibatches, enabling data reuse at the cost of off-policy divergence risk. \textbf{A2C} (centre) collects one episode, makes one gradient update, then discards the data. \textbf{PPO} (right) collects 1,024 steps and reuses that same batch across 10 epochs of minibatch updates before discarding. PPO's multi-epoch reuse amplifies rare goal transitions, which is why it discovers goals on MountainCar while A2C does not.}
\label{fig:rollout}
\end{figure}
...
\end{equation}
Since normalised advantages have exactly zero mean ($\sum_j A_j = 0$), the policy loss was identically zero for every gradient update across all 5,000 episodes. Fix: call \texttt{torch.stack(log\_probs)\allowbreak.squeeze(-1)} to obtain shape \texttt{[T]}, then element-wise multiplication.
...
\centering
\begin{tikzpicture}[
  bx/.style={draw, rectangle, rounded corners=5pt,
             minimum width=8.5cm, minimum height=1.0cm,
             align=center, font=\small, inner sep=6pt},
  arr/.style={->, >=Stealth, line width=1.2pt},
  lbl/.style={font=\footnotesize, fill=white, inner sep=2pt}
]
%% Four boxes, centre-aligned at x=0, separated by 2.0cm
\node[bx, fill=tcdBlue!12]  (p) at (0,  0.0) {Policy $\pi_\theta$ initialised near-uniform};
\node[bx, fill=orange!14]   (c) at (0, -2.2) {Critic fits $V_\pi(s)$ accurately by episode ${\sim}600$};
\node[bx, fill=red!12]      (a) at (0, -4.4) {$\hat{A}(s,a)=Q(s,a)-V_\pi(s)\approx 0$ for all transitions};
\node[bx, fill=gray!14]     (g) at (0, -6.6) {$\nabla_{\!\theta}\,\mathcal{L}_{\text{actor}}\approx 0$ \quad policy gradient vanishes};
%% Downward arrows (right side of centre-line)
\draw[arr] (p.south) -- (c.north)
           node[lbl, midway, right=6pt]{collect rollouts};
\draw[arr] (c.south) -- (a.north)
           node[lbl, midway, right=6pt]{compute GAE advantages};
\draw[arr] (a.south) -- (g.north)
           node[lbl, midway, right=6pt]{normalise advantages};
%% Return arrow on the LEFT side, clear of all text
\draw[arr] (g.west)  -- ++(-2.2,0)  coordinate (bl)
           node[lbl, midway, yshift=-6pt]{no policy update};
\draw[arr] (bl) -- ++(0, 6.6) -- (p.west);
\end{tikzpicture}
\caption{The zero-advantage fixed point in single-environment A2C. Once the critic converges to the current bad policy's value function, advantages vanish, the policy gradient becomes zero, and the policy cannot improve. The actor loss in our experiments reaches $0.000023$ by episode 600 and remains at ${\approx}0$ for all 5,000 episodes.}
