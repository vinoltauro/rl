...
\begin{enumerate}
  ...
  \item \textbf{A documented debugging audit of 43 implementation bugs}, each classified by severity, root cause, and the specific experiment that revealed it. This audit constitutes a practical guide to the failure modes of these four algorithms that goes beyond what any single paper documents.
...
The Policy Gradient Theorem \cite{sutton2018reinforcement} proves that the gradient of expected return with respect to policy parameters can be computed without differentiating the state distribution---a result that makes A2C and PPO mathematically tractable for continuous and discrete control alike.
...
Double DQN & 5,000 ep  & $-174.7 \pm 23.5$ & $-146.7 \pm 6.3$ & $\sim -143$ & Yes \\
A2C        & 5,000 ep  & $-200.0 \pm 0.0$  & $-200.0 \pm 0.0$ & $-200.0$   & \textbf{Zero} \\
PPO        & 8,000 ep  & $-162.8 \pm 36.9$ & $-151.8 \pm 29.4$ & $\sim -148$ & 4,816 \\
\bottomrule
\end{tabularx}
\end{table}
...
\textbf{Double DQN} achieves a stable last-100 average of $-146.7 \pm 6.3$ at episode 5,000---the most stable DQN result after multiple ablation rounds. The agent consistently reaches the goal in approximately 147 steps on average, but cannot close the gap to the 110-step threshold.
...
\textbf{PPO} (v5) is the best result: $-151.8 \pm 29.4$ at episode 8,000 with 4,816 goal discoveries and no late-stage regression. The LR schedule (decaying from $3\times10^{-4}$ to a floor of $5\times10^{-5}$) prevents the catastrophic policy destruction observed in earlier runs where LR decayed to zero. PPO discovers goals in dense bursts from episode ~1,000 onwards and maintains consistent goal-reaching throughout the 8,000-episode run.

\subsubsection{Benchmark Contextualisation}

The RL Baselines3 Zoo reports that PPO on MountainCar-v0 requires 500,000--1,000,000 timesteps using 16 parallel environments and `n\_steps=16`. The v5 PPO implementation uses $\sim1.3\mathrm{M}$ steps (8,000 episodes $\times$ 163 average steps) from a single environment---within the step budget, but without the exploration diversity that parallelism provides. Double DQN from the zoo reports highly variable results dependent on replay buffer configuration; the $-146.7$ stable result achieved here is competitive.
...
Over the course of this project, 43 implementation bugs were identified and corrected. This section documents the most impactful, categorised by algorithm and theoretical significance. The complete audit is maintained in \texttt{EXPERIMENTS\_LOG.md}.
...
\textbf{Bug \#25}: Entropy was estimated as $-\mathbb{E}[\log\pi(a|s)]$ from sampled actions---a single-sample estimator with high variance, biased when the sampled action is not representative. Fix: \texttt{dist.entropy().mean()} computes the exact closed-form entropy $\mathcal{H}[\pi] = -\sum_a \pi(a|s)\log\pi(a|s)$.
...
\textbf{Bug \#28}: Adding reward shaping to tabular Q-Learning caused a regression from mean $-130$ (raw rewards, 50,000 episodes) to mean $-200$ throughout 44,000 episodes---a complete failure. The shaped reward $h(s) + 100v^2 - 1$ creates a locally optimal policy that oscillates at high velocity without crossing $x = 0.5$. With $\varepsilon = 0.001$ (decayed minimum, ep.\ 11,000 onward), the Q-table has no exploration budget to escape this attractor. Neural network methods escape via implicit gradient noise from function approximation; tabular methods with a converged greedy policy cannot.
...
\textbf{Off-policy vs.\ on-policy.} DQN's experience replay allows each transition to be reused indefinitely, dramatically improving data efficiency over A2C and Q-Learning which discard data after each update. However, DQN requires a replay buffer, target network, and careful epsilon scheduling---implementation complexity that introduces multiple failure modes.
...
The 43-bug audit establishes that implementation detail is not a secondary concern in deep RL---it is the primary determinant of whether an algorithm succeeds. The five most impactful bugs by outcome severity were:
...
Each of these bugs produced an outcome change larger than the claimed improvement of the Double DQN paper over standard DQN on this environment---precisely the phenomenon Henderson et al.\ \cite{henderson2018deep} and Engstrom et al.\ \cite{engstrom2020implementation} warned about.
...
This dissertation has presented a comprehensive empirical evaluation of four reinforcement learning algorithms across three environments, accompanied by a systematic debugging audit of 43 implementation errors.
...
\begin{enumerate}
  ...
  \item \textbf{Implementation bugs are the primary determinant of outcomes.} Forty-three bugs were identified across four algorithms. Five of these individually produced outcome changes larger than the gap between algorithm variants. The outer product policy gradient bug (Bug \#29) produced zero policy gradient for 5,000 episodes without any detectable symptom in loss values.
