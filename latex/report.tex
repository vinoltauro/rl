...
\begin{subfigure}[b]{0.48\linewidth}
  \includegraphics[width=\linewidth]{figures/ppo_trajectory.png}
  \caption{PPO v5 best policy. The agent builds momentum through repeated oscillation before crossing the goal at $x=0.5$.}
  \label{fig:ppo_traj}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\linewidth}
  \includegraphics[width=\linewidth]{figures/dqn_trajectory.png}
  \caption{Double DQN v3 best policy. Similar momentum-building pattern, reaching the goal position in the test episode.}
  \label{fig:dqn_traj}
\end{subfigure}
\caption{MountainCar-v0 test episode trajectories for the two best-performing algorithms. Both learn the counter-intuitive strategy of first driving left to build kinetic energy before ascending to the goal.}
...
Even after correcting all 14 issues affecting A2C, the algorithm fails completely on MountainCar with a single environment. The failure is structural, not implementational.
...
\caption{A2C structural failure on MountainCar. \textit{Left}: v3 (all issues resolved) vs.\ run 4 (outer product error, random exploration only). Both hover at $-200$, confirming the failure is structural. \textit{Right}: A2C v3 actor and critic loss on log scale. The critic converges by episode 600 and the actor loss drops to zero, remaining there for 4,400 episodes.}
...
\end{subfigure}
\caption{PPO implementation fixes. Left: effect on average reward and goal discovery. Right: entropy collapse from LR$\to$0 (Issue \#39) vs.\ stable entropy with LR floor.}
\label{fig:ppo_fixes}
\end{figure}
...
The A2C failure is not a code-level error. It is the empirical demonstration of why the A3C paper \cite{mnih2016asynchronous} was structured around parallelism. In a dense-reward environment like CartPole, A2C works because every episode produces non-trivial advantages. In MountainCar, with all episodes returning $-200$ until the agent learns the momentum-building pattern, the critic converges to a constant-value function and advantages vanish.
