...
def train():
    ...
    env.close()
    return agent, episode_rewards, episode_lengths, policy_losses, value_losses, entropies, clip_fractions
...
def plot_results(episode_rewards, episode_lengths, policy_losses, value_losses,
    ...
    axes[0, 0].plot(episode_rewards, alpha=0.4, color='blue', linewidth=0.5)
    ...
    if policy_losses:
        axes[0, 2].plot(policy_losses, color='red', linewidth=2, marker='o', markersize=3)
    ...
    if value_losses:
        axes[1, 0].plot(value_losses, color='purple', linewidth=2, marker='o', markersize=3)
    ...
    if entropies:
        axes[1, 1].plot(entropies, color='green', linewidth=2, marker='o', markersize=3)
    ...
    if clip_fractions:
        axes[1, 2].plot(clip_fractions, color='brown', linewidth=2, marker='o', markersize=3)
        axes[1, 2].axhline(y=0.2, color='red', linestyle='--', alpha=0.5, label='Clip threshold')
    axes[1, 2].set_title('PPO Clip Fraction')
    axes[1, 2].set_xlabel('Update')
    axes[1, 2].set_ylabel('Fraction')
    axes[1, 2].legend()
    axes[1, 2].grid(True, alpha=0.3)

    plt.suptitle('CartPole PPO', fontsize=14, fontweight='bold')
    plt.tight_layout()
...
def test_agent(agent, n_episodes=20, save_dir=None):
    ...
    print(f"\n  Average: {avg:.2f}  |  Success rate: {success_rate:.0f}%")
...
if __name__ == "__main__":
    ...
    agent, rewards, lengths, pl, vl, ent, clip = train()
    plot_results(rewards, lengths, pl, vl, ent, clip, results_dir)
    test_agent(agent, save_dir=results_dir)
